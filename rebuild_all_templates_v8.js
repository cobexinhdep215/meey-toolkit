...
async function main() {
    // 1. Require the CURRENT templates
    // Before requiring, let's make sure typescript is stripped or we use a regex to read them, or just use ts-node?
    // Actually, require('./src/data/templates.ts') won't work without ts-node if it uses `export const`.
    // But since it's just `export const ... = { ... }`, we can just replace the closing brace of the objects.
...
<div style="max-width:600px; margin: 0 auto; border: 2px dashed #ff6b00; padding: 30px; border-radius: 12px; background: #fffcf8; text-align:center;">
  ...
  <div style="padding:15px;background:#f9fafb;border-radius:8px;font-style:italic;line-height:1.6; border: 1px solid #e2e8f0; text-align:left;">[CONTEXT]</div>
    ...
    let updatedTs = templatesTs;

    // Append to formConfigs
    let formConfigsStr = JSON.stringify(newFormConfigs, null, 2);
    formConfigsStr = formConfigsStr.slice(1, -1) + ','; // Remove { and } and add comma
    updatedTs = updatedTs.replace(/export const formConfigs:.*?=\s*\{/, (match) => {
        return match + '\\n' + formConfigsStr;
    });

    // Append to templateContent
    let templateContentStr = JSON.stringify(newTemplateContent, null, 2);
    templateContentStr = templateContentStr.slice(1, -1) + ','; // Remove { and } and add comma
    updatedTs = updatedTs.replace(/export const templateContent:.*?=\s*\{/, (match) => {
        return match + '\\n' + templateContentStr;
    });

    fs.writeFileSync('src/data/templates.ts', updatedTs);
    console.log("Appended strictly to existing file successfully");
}
