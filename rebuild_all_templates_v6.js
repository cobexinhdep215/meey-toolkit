...
async function main() {
    ...
    // Cleanup messy lines ONLY on their actual tags
    htmlDC = htmlDC.replace(/Ông <strong>[^<]*<\/strong>, Ngày sinh:[^<]*?Nơi thường trú:[^<]*/g, '');
    htmlDC = htmlDC.replace(/Và vợ là bà <strong>[^<]*<\/strong>, Ngày sinh:[^<]*?Nơi thường trú:[^<]*/g, '');
    htmlDC = htmlDC.replace(/\(Hai ông bà là vợ chồng theo Giấy chứng nhận kết hôn số[^<]*?\)/g, '');
...
<div style="max-width:600px; margin: 0 auto; border: 2px dashed #ff6b00; padding: 30px; border-radius: 12px; background: #fffcf8; text-align:center;">
  ...
  <div style="padding:15px;background:#f9fafb;border-radius:8px;font-style:italic;line-height:1.6; border: 1px solid #e2e8f0; text-align:left;">[CONTEXT]</div>
    ...
    // Serialize to valid TS object notations
    templatesTs += "export const formConfigs: Record<string, { name: string, label: string, placeholder: string, mockData: string, isMoney?: boolean }[]> = " + JSON.stringify(formConfigsObj, null, 2) + ";\n\n";
    templatesTs += "export const templateContent: Record<string, string> = " + JSON.stringify(templateContentObj, null, 2) + ";\n";

    fs.writeFileSync('src/data/templates.ts', templatesTs);
    console.log("Rebuilt strictly with JSON.stringify successfully");
}
