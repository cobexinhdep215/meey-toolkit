...
async function main() {
    ...
    htmlMB = htmlMB.replace(/Căn hộ số: [\.…\s]+ tại tầng \(tầng có căn hộ\): [\.…\s]+, thuộc nhà chung cư [\.…\s]+ đường\/phố \(nếu có\)/, 'Căn hộ: <strong>[tenBDS]</strong>, diện tích: <strong>[dienTich] m2</strong>, địa chỉ: <strong>[diaChi]</strong>');
    htmlMB = htmlMB.replace(/Điện thoại liên hệ: [\.…\s]+/, 'Điện thoại liên hệ: <strong>[sdt]</strong>');
    ...
    htmlDC = htmlDC.replace(/Ông <strong>[\.…\s]+<\/strong>, Ngày sinh:.+?Nơi thường trú: [\.…\s]+/, ''); // Cleanup first Ông block
    htmlDC = htmlDC.replace(/Và vợ là bà.+?Nơi thường trú: [\.…\s]+/, ''); // Cleanup vợ bà block
    htmlDC = htmlDC.replace(/\(Hai ông bà là vợ chồng theo Giấy chứng nhận kết hôn số.*?\)/, ''); // Cleanup vợ chồng block

    htmlDC = htmlDC.replace(/Ông <strong>[\.…\s]+<\/strong>, Ngày sinh:.+?Nơi thường trú: [\.…\s]+/, ''); // Cleanup second Ông block
...
'  "Biểu mẫu thanh toán theo đợt": ' + bq, bmDotHTML, bq
    ...
    // Must remove invalid characters otherwise TS compilation fails!
    const cleanTs = newTs.replace(/[\\x00-\\x09\\x0B-\\x1F\\x7F-\\x9F]/g, '');
